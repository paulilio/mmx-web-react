        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-slate-600">Selecione as categorias de despesa que pertencem a este grupo</p>
            </CardHeader>
            <CardContent className="space-y-3">
              {availableCategories.length > 0 ? (
                <div className="grid grid-cols-1 gap-2 max-h-48 overflow-y-auto">
                  {availableCategories.map((category) => (
                    <div key={category.id} className="flex items-center space-x-3 p-2 rounded-lg hover:bg-slate-50">
                      <Checkbox
                ...
                <p className="text-sm text-slate-500 text-center py-4">Nenhuma categoria de despesa disponível</p>
              )}

              {selectedCategories.length > 0 && (
                <div className="mt-3 pt-3 border-t">
                  <p className="text-sm text-slate-600 mb-2">{selectedCategories.length} categoria(s) selecionada(s)</p>
                </div>
              )}
            </CardContent>
          </Card>
