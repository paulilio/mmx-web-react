...
## Concept
Todo acesso a dados no frontend deve passar obrigatoriamente pela cadeia: UI → hooks → lib/client/api.ts → apps/api. Nenhum componente ou página acessa storage, repositórios ou backend diretamente.
...
2. Hook usa lib/client/api.ts para chamadas HTTP
3. lib/client/api.ts desembrulha envelope { data, error }
...
- Fazer fetch direto sem passar por lib/client/api.ts
- Criar mock fallback automático em API mode

## Example in This Project
- lib/client/api.ts:handleResponse — desembrulha envelope, erro explícito
- lib/client/api.ts:requestApi — credentials: "include" para requests externos
- hooks/use-auth.tsx — login/logout/refresh integrados com /api/auth/*
- hooks/use-settings-maintenance.ts — settings via /api/settings/* (criado em tk-103)
- app/settings/page.tsx — removido bypass de lib/server/storage (tk-103)
