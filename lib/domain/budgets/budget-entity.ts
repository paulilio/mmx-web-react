import { validateMonth, validateYear, validateNonNegativeAmount, normalizeRolloverEnabled } from "./budget-rules"
...
export class BudgetEntity {
  ...
  buildUpdatePayload(input: UpdateBudgetInput): Partial<BudgetEntityProps> {
    if (typeof input.planned === "number") validateNonNegativeAmount(input.planned, "planned")
    if (typeof input.funded === "number") validateNonNegativeAmount(input.funded, "funded")

    return {
      planned: input.planned ?? undefined,
      funded: input.funded ?? undefined,
      rolloverEnabled: input.rolloverEnabled == null ? undefined : normalizeRolloverEnabled(input.rolloverEnabled),
