...
import type { ISettingsMaintenanceRepository } from "../../application/ports/settings-maintenance-repository.port"
...
function normalizeActiveInactiveStatus(
  value: unknown,
  kind: "area" | "category" | "group" | "contact",
): "ACTIVE" | "INACTIVE" {
  if (typeof value !== "string") {
...
@Injectable()
export class PrismaSettingsMaintenanceRepository
  implements ISettingsMaintenanceRepository
{
  private readonly db: SettingsDbClient

  constructor(private readonly prisma: PrismaService) {
    this.db = prisma as unknown as SettingsDbClient
  }
  ...
  async exportData(userId: string, tables?: unknown[]): Promise<Partial<SeedData>> {
    ...
    if (selectedTables.includes("mmx_areas")) {
      result.mmx_areas = await this.db.area.findMany({
        where: { userId },
    ...
    if (selectedTables.includes("mmx_category_groups")) {
      result.mmx_category_groups = await this.db.categoryGroup.findMany({
        where: { userId },
        orderBy: { createdAt: "asc" },
      })
    }
    ...
    if (selectedTables.includes("mmx_categories")) {
      result.mmx_categories = await this.db.category.findMany({
        where: { userId },
        orderBy: { createdAt: "asc" },
      })
    }
    ...
    if (selectedTables.includes("mmx_transactions")) {
      result.mmx_transactions = await this.db.transaction.findMany({
        where: { userId },
        orderBy: { date: "asc" },
      })
    }
    ...
    if (selectedTables.includes("mmx_contacts")) {
      result.mmx_contacts = await this.db.contact.findMany({
        where: { userId },
        orderBy: { createdAt: "asc" },
      })
    }
        ...
        const created = await tx.categoryGroup.createMany({
          data: categoryGroups.map((item) => ({
            ...
            categoryIds: Array.isArray(item.categoryIds)
              ? item.categoryIds.filter((id) => typeof id === "string")
              : [],
            createdAt: parseOptionalDate(item.createdAt),
            updatedAt: parseOptionalDate(item.updatedAt),
          })),
      ...
      if (transactions.length > 0) {
        const created = await tx.transaction.createMany({
          data: transactions.map((item) => ({
            ...
            recurrence:
              isRecord(item.recurrence) || Array.isArray(item.recurrence)
                ? item.recurrence
                : undefined,
            areaId: typeof item.areaId === "string" ? item.areaId : null,
            categoryGroupId:
