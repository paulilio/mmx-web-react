...
import { useGruposCategorias } from "@/hooks/compat/use-grupos-categorias"
...
export function GrupoCategoriaFormModal({ open, onOpenChange, grupoCategoria }: GrupoCategoriaFormModalProps) {
  const [isSubmitting, setIsSubmitting] = useState(false)

  const { createGrupoCategoria, updateGrupoCategoria } = useGruposCategorias()
  const { areas } = useAreas()

  const form = useForm<GrupoCategoriaFormData>({
    resolver: zodResolver(grupoCategoriaSchema),
    defaultValues: {
      name: "",
      description: "",
  ...
  useEffect(() => {
    if (open) {
      if (grupoCategoria) {
        reset({
          name: grupoCategoria.name,
          description: grupoCategoria.description || "",
          color: grupoCategoria.color,
          icon: grupoCategoria.icon,
          status: grupoCategoria.status,
          areaId: grupoCategoria.areaId,
        })
      } else {
        reset({
          name: "",
  ...
  }, [open, grupoCategoria, reset])

  const onSubmit = async (data: GrupoCategoriaFormData) => {
    setIsSubmitting(true)
    try {
      if (grupoCategoria) {
        await updateGrupoCategoria(grupoCategoria.id, data)
      } else {
        await createGrupoCategoria(data)
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" key={grupoCategoria?.id || "new"}>
        <DialogHeader>
          <DialogTitle>{grupoCategoria ? "Editar Grupo Categoria" : "Novo Grupo Categoria"}</DialogTitle>
        </DialogHeader>
