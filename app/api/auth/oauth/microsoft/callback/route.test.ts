...
const { oauthAuthServiceMock, microsoftOAuthServiceMock } = vi.hoisted(() => ({
  oauthAuthServiceMock: {
    loginWithMicrosoftProfile: vi.fn(),
  },
...
describe("/api/auth/oauth/microsoft/callback", () => {
  ...
  it("cria usuario quando email nao existe", async () => {
    microsoftOAuthServiceMock.exchangeMicrosoftCodeForProfile.mockResolvedValueOnce({
      ...
      oauthAuthServiceMock.loginWithMicrosoftProfile.mockResolvedValueOnce({
        isNewUser: true,
        user: {
          id: "u-ms-1",
          email: "novo-ms@mmx.com",
    ...
    expect(payload.data.isNewUser).toBe(true)
      expect(oauthAuthServiceMock.loginWithMicrosoftProfile).toHaveBeenCalledTimes(1)
  })
